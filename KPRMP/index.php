...
$salesList = [];
$allGoodsForPOS = $pdo->query("SELECT * FROM retail_goods ORDER BY name ASC")->fetchAll();
if ($userRole === 'admin' || $userRole === 'developer' || $userRole === 'ritel' || $userRole === 'pengawas') {
    $salesList = $pdo->query("SELECT s.*, m.name as member_name, m.member_code 
                              FROM retail_sales s
...
<div class="app-wrapper">
    ...
    <div class="main-layout">
        ...
        <main class="content-body">
            ...
            <!-- =================================================== -->
            <!-- 3b. PANEL: UNIT USAHA KASIR -->
            <!-- =================================================== -->
            <section id="kasir" class="module-panel">
                <!-- Sub Tab Navigation -->
                <div class="card-frame" style="margin-bottom: 25px; padding: 15px 24px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px;">
                        <div style="display:flex; gap:10px;">
                            <button id="btn-cashier-pos" class="btn-primary" onclick="switchCashierTab('pos')">🛒 Mesin Kasir</button>
                            <button id="btn-cashier-history" class="btn-secondary" onclick="switchCashierTab('history')">📜 Riwayat Transaksi</button>
                        </div>
                        <div>
                            <span class="badge b-info" style="font-size:0.85rem; padding: 6px 12px; font-weight:600;">
                                Kasir: <?= htmlspecialchars($userName) ?> (<?= htmlspecialchars(strtoupper($userRole)) ?>)
                            </span>
                        </div>
                    </div>
                </div>

                <!-- Sub Tab 1: POS Interface -->
                <div id="cashier-pos-container">
                    <div class="cashier-layout" style="display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 25px;">
                        <!-- Left: Products List -->
                        <div class="card-frame" style="display: flex; flex-direction: column; gap: 20px;">
                            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px;">
                                <h3 class="card-frame-title">Daftar Produk Toko</h3>
                                <div style="display: flex; gap: 10px; flex: 1; max-width: 450px;">
                                    <input type="text" id="pos-search" class="form-control" placeholder="Cari nama atau kode barang..." oninput="renderPOSProducts()" style="padding: 8px 12px; font-size: 0.85rem;">
                                    <select id="pos-category-filter" class="form-control" onchange="renderPOSProducts()" style="width: 150px; padding: 8px 12px; font-size: 0.85rem;">
                            ...
                            <div id="pos-products-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 15px; max-height: 520px; overflow-y: auto; padding-right: 5px;">
                                <!-- Rendered dynamically by JS -->
                            </div>
                        </div>

                        <!-- Right: Cart & Checkout -->
                        <div class="card-frame" style="display: flex; flex-direction: column; justify-content: space-between; gap: 20px;">
                            <div>
                                <h3 class="card-frame-title" style="margin-bottom: 15px;">Detail Transaksi Belanja</h3>
                                <div style="max-height: 250px; overflow-y: auto; border: 1px solid var(--border-light); border-radius: 12px; margin-bottom: 15px;">
                                    <table class="modern-table" style="font-size: 0.85rem;">
                                        <thead>
                                            <tr>
                                                <th style="padding: 10px 12px;">Barang</th>
                                                <th style="padding: 10px 12px; text-align: center; width: 100px;">Qty</th>
                                                <th style="padding: 10px 12px; text-align: right;">Total</th>
                                                <th style="padding: 10px 12px; text-align: center;"></th>
                                            </tr>
                                        </thead>
                                        <tbody id="pos-cart-tbody">
                                            <!-- Cart Items Rendered dynamically by JS -->
                                ...
                                <div style="display: flex; flex-direction: column; gap: 12px; padding: 15px; background: var(--bg-main); border-radius: 12px; margin-bottom: 15px;">
                                    <div style="display: flex; justify-content: space-between; font-weight: 600;">
                                        <span>Total Belanja:</span>
                                        <span style="color: var(--primary); font-size: 1.1rem;" id="pos-total-display">Rp 0</span>
                                    </div>
                                    <hr style="border: none; border-top: 1px solid var(--border-light);">
                                    <div class="form-group" style="gap: 5px;">
                                        <label style="font-size: 0.75rem; font-weight: 700;">Tipe Pembeli</label>
                                        <select id="pos-customer-type" class="form-control" onchange="toggleCustomerType()" style="padding: 8px 12px; font-size: 0.85rem;">
                                            <option value="Umum">Umum / Non-Anggota</option>
                                            <option value="Anggota">Anggota Koperasi</option>
                                        </select>
                                    </div>
                                    <div class="form-group" id="pos-member-select-group" style="display: none; gap: 5px;">
                                        <label style="font-size: 0.75rem; font-weight: 700;">Pilih Anggota</label>
                                        <select id="pos-member-id" class="form-control" style="padding: 8px 12px; font-size: 0.85rem;">
                                            <option value="" disabled selected>-- Pilih Anggota --</option>
                                            <?php foreach ($membersList as $m): ?>
                                                <?php if ($m['status'] === 'Aktif'): ?>
                                                    <option value="<?= $m['id'] ?>"><?= htmlspecialchars($m['member_code']) ?> - <?= htmlspecialchars($m['name']) ?></option>
                                                <?php endif; ?>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                ...
                                <div style="display: flex; flex-direction: column; gap: 10px; padding: 15px; background: rgba(12, 166, 120, 0.04); border: 1px solid rgba(12, 166, 120, 0.15); border-radius: 12px;">
                                    <div class="form-group" style="gap: 5px;">
                                        <label style="font-size: 0.75rem; font-weight: 700; color: var(--success);">Uang Bayar (Rp)</label>
                                        <input type="number" id="pos-payment" class="form-control" placeholder="Contoh: 100000" oninput="calculateChange()" style="padding: 10px 14px; font-size: 1rem; font-weight: 700; border-color: rgba(12, 166, 120, 0.25);">
                                    </div>
                                    <div style="display: flex; justify-content: space-between; font-weight: 700; color: var(--success); font-size: 1rem; margin-top: 5px;">
                                        <span>Kembalian:</span>
                                        <span id="pos-change-display">Rp 0</span>
                                    </div>
                                </div>
                            </div>

                            <button class="btn-primary" onclick="submitPOSCheckout()" style="width: 100%; padding: 14px; justify-content: center; border-radius: 12px; font-size: 0.95rem; font-weight: 700; gap: 10px; margin-top: 15px;">
                ...
                <div id="cashier-history-container" style="display: none;">
                    <div class="card-frame">
                        <div class="card-frame-header">
                            <h3 class="card-frame-title">Riwayat Transaksi Penjualan Ritel</h3>
                        </div>
                        <div class="table-responsive-wrapper">
                            <table class="modern-table">
                                <thead>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td colspan="8" style="text-align: center; color: var(--text-light); padding: 30px 0;">Belum ada riwayat transaksi</td>
                                    </tr>
                                    <?php else: ?>
                                        <?php foreach ($salesList as $sale): ?>
                                        <tr>
                                            <td style="font-weight: 600;"><?= htmlspecialchars($sale['transaction_code']) ?></td>
                                            <td><?= htmlspecialchars($sale['created_at']) ?></td>
                                            <td><?= htmlspecialchars($sale['created_by']) ?></td>
                                            <td>
                                                <?php if (!empty($sale['member_id'])): ?>
                                                    <span class="badge b-info" title="Kode: <?= htmlspecialchars($sale['member_code'] ?? '') ?>">
                                                        👤 <?= htmlspecialchars($sale['member_name'] ?? 'Anggota') ?>
                                                    </span>
                                                <?php else: ?>
                                                    <span class="badge b-grey">Umum</span>
                                                <?php endif; ?>
                                            </td>
                                            <td style="font-weight: 600;">Rp <?= number_format($sale['total_amount'], 0, ',', '.') ?></td>
                                            <td>Rp <?= number_format($sale['payment_amount'], 0, ',', '.') ?></td>
                                            <td>Rp <?= number_format($sale['change_amount'], 0, ',', '.') ?></td>
                                            <td style="text-align: center; white-space: nowrap;">
                                                <button class="btn-icon bi-edit" style="background: rgba(12, 166, 120, 0.08); color: var(--success);" title="Cetak Ulang Struk" onclick="printReceiptDirect(<?= $sale['id'] ?>)">🖨️</button>
                                                <?php if ($userRole === 'admin' || $userRole === 'developer'): ?>
                                                <button class="btn-icon bi-delete" title="Batalkan Transaksi (Retur)" onclick="deleteSale(<?= $sale['id'] ?>, '<?= htmlspecialchars($sale['transaction_code']) ?>')">🗑️</button>
                                                <?php endif; ?>
                                            </td>
                                        </tr>
                                        <?php endforeach; ?>
...
<div class="modal-overlay" id="receipt-modal">
    <div class="modal-card" style="max-width: 420px; border-radius: 12px;">
        ...
        <div class="modal-body" style="padding: 20px; background: #f8f9fa;">
            ...
            <div id="printable-receipt" style="font-family: 'Courier New', Courier, monospace; font-size: 0.85rem; color: #000; line-height: 1.4; background: #fff; padding: 20px; border: 1px dashed #bbb; border-radius: 4px; box-shadow: inset 0 0 10px rgba(0,0,0,0.02);">
                <!-- Filled dynamically by JS -->
            </div>
        </div>
        <div class="modal-footer" style="padding: 15px 20px;">
            <button class="btn-secondary" onclick="closeReceiptModal()" style="padding: 8px 16px;">Tutup</button>
            <button class="btn-primary" onclick="printReceiptContent()" style="padding: 8px 16px; background: var(--success); box-shadow: none;">🖨️ Cetak Struk</button>
        </div>
    </div>
</div>
...
<!-- Global POS Data -->
<script>
    const posGoodsData = <?= json_encode($allGoodsForPOS) ?>;
    const posMembersData = <?= json_encode($membersList) ?>;
</script>

<!-- Custom Scripts -->
<script src="assets/js/main.js?v=1.3"></script>
<script src="assets/js/charts.js?v=1.3"></script>
</body>
</html>
